...
if (!uri) {
  ...
  console.error(
    '$env:MONGODB_URI = "mongodb+srv://user:pass@cluster0/yourdb"; node .\\assign-invoice-ids.js --yes',
  );
  process.exit(1);
}
...
const skipExisting =
  argv.includes("--skip-existing") || argv.includes("--skipExisting");
...
async function run() {
  ...
  try {
    ...
    if (skipExisting) {
      ...
      if (!applyChanges) {
        console.log(
          `Dry run complete. ${wouldUpdate} invoices would be updated. Re-run with --yes to apply.`,
        );
      } else {
