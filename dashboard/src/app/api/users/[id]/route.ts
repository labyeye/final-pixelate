...
export async function DELETE(request: Request, context: RouteContext) {
	try {
		...
		try {
			...
			if (ObjectId.isValid(id)) assignedToFilters.push({ assignedTo: new ObjectId(id) });
			await leadsCol.updateMany({ $or: assignedToFilters }, { $unset: { assignedTo: '', assignedToName: '' } });
			...
			await quotationsCol.updateMany({ authorId: { $in: authorIds } }, { $unset: { authorId: '' } });
		} catch (e) {
