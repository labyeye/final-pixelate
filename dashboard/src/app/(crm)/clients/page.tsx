...
import {
  MoreVertical, ShieldCheck, Search, Filter, X, Download,
  ChevronRight, Users, UserCheck, TrendingUp, UserX, Plus,
} from "lucide-react";
      ...
      <div className="bg-white border-4 border-black rounded-2xl shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] px-10 py-8 flex flex-col items-center gap-4 animate-in zoom-in-90 duration-200">
        <div className="relative flex items-center justify-center">
          <svg width="72" height="72" viewBox="0 0 72 72" fill="none">
            ...
            <polyline points="20,37 31,48 52,26" fill="none" stroke="white" strokeWidth="5" strokeLinecap="round" strokeLinejoin="round"
              style={{ strokeDasharray: 50, strokeDashoffset: 0, animation: "draw-check 0.4s ease forwards" }} />
          </svg>
...
export default function ClientsPage() {
  ...
  const filtered = useMemo(() => {
    return clients.filter(c => {
      if (search) {
        ...
        const match = [c.name, c.email, c.phone, c.city, c.state, c.gstNumber]
  ...
  function exportCSV() {
    const rows = [
      ["Name", "Email", "Phone", "City", "State", "Status", "Industry", "Tags", "GST", "Portal"],
      ...filtered.map(c => [
        c.name, c.email || "", c.phone || "", c.city || "", c.state || "",
        c.status || "active", c.industry || "",
        (c.tags || []).join("; "),
  ...
  const clearFilters = () => { setSearch(""); setStatusFilter(""); setTagFilter(""); };
    ...
    <div className="space-y-6 font-headline">
      ...
      <header className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-muted-foreground text-lg">{filtered.length} of {clients.length} clients</p>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          <Button variant="outline" size="sm" onClick={exportCSV} className="border-2 border-black font-bold">
            <Download className="w-4 h-4 mr-1" /> Export
          </Button>
          <AddClientDialog isOpen={isDialogOpen} setIsOpen={setIsDialogOpen} onAddClient={handleAddClient}>
            <Button size="sm" className="border-2 border-black font-bold text-base">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          ...
          { label: "Prospect", value: prospect, icon: TrendingUp, color: "bg-blue-50 border-blue-300", filter: "prospect" },
          { label: "Inactive", value: inactive, icon: Users, color: "bg-gray-50 border-gray-300", filter: "inactive" },
          { label: "Churned", value: churned, icon: UserX, color: "bg-red-50 border-red-300", filter: "churned" },
        ].map(s => {
          const Icon = s.icon;
          return (
            <button
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        >
          ...
          {hasFilters && <span className="ml-1 bg-red-500 text-white text-xs rounded-full w-4 h-4 flex items-center justify-center">!</span>}
        </Button>
        {hasFilters && (
      ...
      {showFilters && (
        <div className="border-2 border-black rounded-xl p-4 grid grid-cols-2 sm:grid-cols-3 gap-3 bg-gray-50">
          <div>
            <label className="text-xs font-bold mb-1 block uppercase tracking-wide">Status</label>
            <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} className="w-full px-2 py-1.5 rounded-lg border-2 border-black text-sm bg-white">
          ...
          {allTags.length > 0 && (
            <div>
              <label className="text-xs font-bold mb-1 block uppercase tracking-wide">Tag</label>
              <select value={tagFilter} onChange={e => setTagFilter(e.target.value)} className="w-full px-2 py-1.5 rounded-lg border-2 border-black text-sm bg-white">
              ...
              return (
                ...
                >
                  <TableCell className="font-bold text-base py-4">
                    ...
                    {c.industry && <div className="text-xs text-muted-foreground font-normal">{c.industry}</div>}
                  </TableCell>
                  ...
                  <TableCell className="py-4">
                    <div className="flex flex-wrap gap-1">
                      {(c.tags || []).slice(0, 3).map(tag => (
                        <span key={tag} className="text-xs px-1.5 py-0.5 rounded-full bg-purple-50 text-purple-700 border border-purple-200">
                          #{tag}
                        </span>
                      ))}
                      {(c.tags || []).length > 3 && (
                    ...
                    <div className="flex items-center justify-end gap-2" onClick={e => e.stopPropagation()}>
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          <DropdownMenuItem onClick={() => router.push(`/clients/${cid}`)}>View</DropdownMenuItem>
