...
import AddExpenseDialog, { EXPENSE_CATEGORIES, PAYMENT_METHODS } from '@/components/expenses/add-expense-dialog';
import {
  TrendingUp,
  TrendingDown,
  Wallet,
...
const STATUS_CONFIG: Record<string, { label: string; icon: any; class: string }> = {
  paid: { label: 'Paid', icon: CheckCircle2, class: 'bg-success/10 text-success border-success/30' },
  pending: { label: 'Pending', icon: AlertCircle, class: 'bg-secondary/10 text-secondary border-secondary/30' },
  cancelled: { label: 'Cancelled', icon: XCircle, class: 'bg-destructive/10 text-destructive border-destructive/30' },
};
...
function StatusBadge({ status }: { status: string }) {
  ...
  const Icon = cfg.icon;
  return (
    <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full border text-xs font-semibold ${cfg.class}`}>
    ...
    <div className="space-y-6 font-headline">
      ...
      <header className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Complete business expense management — salary, inventory, office & more.</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <CardContent className="pt-4 pb-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-black mt-1">₹{stats.total.toLocaleString()}</p>
                <p className="text-xs text-muted-foreground mt-0.5">{stats.count} entries</p>
              </div>
        ...
        <Card className="border-2 border-black">
          <CardContent className="pt-4 pb-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs text-muted-foreground font-semibold uppercase tracking-wide">This Month</p>
                <p className="text-2xl font-black mt-1">₹{stats.thisMonthTotal.toLocaleString()}</p>
                <p className="text-xs text-muted-foreground mt-0.5">Paid expenses</p>
              </div>
          ...
          <CardContent className="pt-4 pb-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-black mt-1">₹{stats.pendingTotal.toLocaleString()}</p>
                <p className="text-xs text-muted-foreground mt-0.5">To be paid</p>
              </div>
        ...
        <Card className="border-2 border-black">
          <CardContent className="pt-4 pb-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs text-muted-foreground font-semibold uppercase tracking-wide">Top Category</p>
                <p className="text-lg font-black mt-1 capitalize leading-tight">
                  {stats.topCategory ? getCategoryLabel(stats.topCategory[0]).split(' ').slice(1).join(' ') : '—'}
                </p>
                <p className="text-xs text-muted-foreground mt-0.5">
                  {stats.topCategory ? `₹${Number(stats.topCategory[1]).toLocaleString()}` : 'No data'}
                </p>
              </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <div className="overflow-x-auto pb-1">
          <TabsList className="inline-flex h-auto gap-1 bg-transparent p-0 flex-wrap">
            <TabsTrigger value="all" className="border-2 border-black data-[state=active]:bg-black data-[state=active]:text-white rounded-lg px-3 py-1.5 text-sm font-bold">
              All
            </TabsTrigger>
            {EXPENSE_CATEGORIES.map(cat => (
              <TabsTrigger key={cat.value} value={cat.value} className="border-2 border-black data-[state=active]:bg-black data-[state=active]:text-white rounded-lg px-3 py-1.5 text-sm font-bold whitespace-nowrap">
                {cat.label}
              </TabsTrigger>
            ))}
          </TabsList>
              ...
              <CardHeader className="pb-3 flex flex-row items-center justify-between">
                <div>
                  <CardTitle className="text-lg font-black">
                    {tabVal === 'all' ? 'All Expenses' : getCategoryLabel(tabVal)}
                  </CardTitle>
                  <CardDescription>
                    {filtered.length} records · Total Paid: <span className="font-bold text-black">₹{filteredTotal.toLocaleString()}</span>
                  </CardDescription>
                </div>
              </CardHeader>
                    ...
                    <Table>
                      <TableHeader>
                        <TableRow className="bg-muted/50">
                          ...
                          <TableHead className="font-black text-right">Date</TableHead>
                      ...
                      <TableBody>
                        {filtered.map(e => (
                          <TableRow key={e._id ?? e.id} className="hover:bg-muted/30 transition-colors">
                            ...
                            <TableCell className="text-sm">{getPaymentLabel(e.paymentMethod) || '—'}</TableCell>
                            <TableCell>
