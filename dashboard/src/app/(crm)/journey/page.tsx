...
function AddEventDialog({ open, onClose, clientId, clientName, onAdded }: AddEventDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-lg border-2 border-black shadow-[6px_6px_0px_0px_rgba(0,0,0,1)]">
        <DialogHeader>
          <DialogTitle className="font-black text-xl tracking-tight">Add Journey Event</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-4 py-2">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1.5">
              ...
              <Select value={form.type} onValueChange={(v) => set('type', v as JourneyEventType)}>
            ...
            <div className="space-y-1.5">
              ...
              <Select value={form.status} onValueChange={(v) => set('status', v)}>
                ...
                <SelectContent>
                  <SelectItem value=" " className="font-bold">None</SelectItem>
        ...
        <DialogFooter className="gap-2">
          <Button variant="outline" className="border-2 border-black font-bold" onClick={onClose}>
            Cancel
          </Button>
          <Button
...
function JourneyCard({ event }: { event: JourneyEvent }) {
  ...
  return (
    <div className="flex gap-3 group">
      ...
      <div
        className={cn(
          'flex-1 rounded-xl border-2 border-black bg-white shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] p-4 transition-shadow group-hover:shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]',
        )}
      >
        {/* Header row */}
            ...
            {event.status && event.status.trim() && (
              <span
                className={cn(
                  ...
                  STATUS_VARIANT[event.status] ?? 'bg-gray-100 text-gray-700 border-gray-200',
                )}
              >
                {event.status}
              </span>
        ...
        {/* Title */}
        <h3 className="font-black text-base tracking-tight mt-1">{event.title}</h3>
        ...
        <div className="flex items-center gap-1.5 mt-3 pt-2 border-t border-dashed border-gray-200">
          ...
          <span className="text-xs font-bold text-muted-foreground">{event.performedBy ?? 'System'}</span>
        </div>
      </div>
    </div>
  );
        ...
        <div className="max-w-3xl mx-auto px-4 py-6">
          {loading ? (
            <div className="flex flex-col items-center justify-center py-32 gap-3">
              ...
              <p className="font-bold text-muted-foreground">Loading journey…</p>
            </div>
          ) : groups.length === 0 ? (
            ...
            <div className="space-y-2">
              {groups.map((group) => (
                <div key={group.label + group.date.toISOString()} className="space-y-3">
                  <DateSeparator label={group.label} date={group.date} />
                  <div className="space-y-3">
                    {group.events.map((ev) => (
                      <JourneyCard key={String(ev._id ?? ev.occurredAt)} event={ev} />
                    ))}
                  </div>
                </div>
              ))}
