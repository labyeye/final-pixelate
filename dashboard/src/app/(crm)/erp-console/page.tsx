  ...
  return (
    <div className="p-6 min-h-screen bg-white text-slate-900">
      ...
      <Card className="bg-white border-slate-200">
        <CardHeader>
          <CardTitle className="font-mono text-slate-900">Recent Events</CardTitle>
          <CardDescription className="font-mono text-slate-600">Raw event stream (newest first)</CardDescription>
        </CardHeader>
        <CardContent className="p-0">
          <div className="max-h-[60vh] overflow-auto font-mono text-sm bg-white text-slate-900 p-4">
            {loading && <div className="text-slate-400">Loading...</div>}
            {!loading && events.length === 0 && <div className="text-slate-500">No events yet.</div>}
            ...
            {events.map((ev) => {
              ...
              if (ev.type === "login") {
                ...
                const pagesList = Object.entries(pagePerms).map(([page, perms]: [string, any]) => {
              ...
              return (
                <div key={ev._id ?? ev.id} className="mb-4 pb-4 border-b border-slate-200 last:border-b-0">
                  <div className="flex items-center gap-3 flex-wrap">
                    <div className="text-slate-500 text-xs">{t}</div>
                    <div className={`px-2 py-0.5 rounded-sm text-xs font-semibold ${typeClass}`}>{ev.type}</div>
                    <div className="text-slate-700 font-medium">{actionMsg}</div>
                  </div>
                  {details && (
