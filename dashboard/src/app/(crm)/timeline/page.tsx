...
export default function TimelinePage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!mounted) return;
        const groupedProjects = projectStatuses.reduce(
          (acc, status) => {
            acc[status] = (projects as Project[]).filter(
              (p) => p.status === status,
            );
            return acc;
          },
  ...
  const onDragEnd = (result: DropResult) => {
    ...
    if (sourceStatus === destStatus) {
      ...
      const updatedDestItems = (resultMap[destStatus] ?? []).map((item) => ({
        ...item,
        status: destStatus,
      }));
  ...
  const handleAssigneeChange = (
    ...
    for (const status in newProjectData) {
      const projectIndex = newProjectData[status as ProjectStatus].findIndex(
      ...
      if (projectIndex !== -1) {
        ...
        if (
          currentAssignees.some(
            (a: any) => String(a.id ?? a) === String(memberId),
          )
        ) {
          project.assignees = currentAssignees.filter(
            (a: any) => String(a.id ?? a) !== String(memberId),
          );
        } else {
          project.assignees = [
            ...currentAssignees,
            { id: memberId, payout: 0 },
          ];
        }
        break;
      }
    ...
    <div className="space-y-8 font-headline">
      <header>
        ...
        <p className="text-muted-foreground text-lg">
          Manage your projects from backlog to completion.
        </p>
      </header>
                              ...
                              >
                                ...
                                >
                                  <CardHeader>
                                    ...
                                    <CardTitle className="text-xl font-bold tracking-tight pt-2">
                                      {project.title}
                                    </CardTitle>
                                  </CardHeader>
                                  <CardContent>
                                      ...
                                      <DropdownMenu>
                                        ...
                                        <DropdownMenuContent align="end">
                                          {teamMembers.map((member) => (
                                            <DropdownMenuCheckboxItem
                                              key={member.id}
                                              checked={project.assignees?.some(
                                                (a: any) =>
                                                  String(a.id ?? a) ===
                                                  String(member.id),
                                              )}
