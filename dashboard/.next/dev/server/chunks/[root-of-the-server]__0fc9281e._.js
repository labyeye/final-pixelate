...
"[project]/Desktop/Projects/final-pixelate/dashboard/src/app/api/social-media-posts/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function GET(request) {
    try {
        ...
        const items = await col.find(query).sort({
            scheduledDate: 1,
            scheduledTime: 1,
            createdAt: -1
        }).toArray();
...
async function POST(request) {
    try {
        ...
        const toInsert = {
            ...
            carouselMediaLinks: body?.carouselMediaLinks || [],
            reelLink: body?.reelLink || "",
            reelCoverLink: body?.reelCoverLink || "",
            scheduledDate: body?.scheduledDate || "",
            scheduledTime: body?.scheduledTime || "",
            assignedTo: body?.assignedTo || "",
            status: body?.status || "Draft",
            approvalStatus: body?.approvalStatus || (body?.status === "Scheduled" ? "Approved" : "Pending"),
            notes: body?.notes || "",
...
async function PUT(request) {
    try {
        ...
        if (changedFields.length > 0) {
            await logErpEvent("post_updated", `post_${id}`, {
                postId: id,
...
//# sourceMappingURL=%5Broot-of-the-server%5D__0fc9281e._.js.map