module.exports = [
  ...
  "[project]/Desktop/Projects/final-pixelate/dashboard/src/app/api/quotations/[id]/route.ts [app-route] (ecmascript)",
  (__turbopack_context__) => {
    "use strict";
        ...
        if (clientId && prevStatus === "DRAFT" && newStatus !== "DRAFT") {
          try {
            ...
            if (!alreadyExists) {
              ...
              const grandTotal = (q.services ?? []).reduce(
              ...
              if ((q.scope ?? []).length > 0)
                descParts.push(`🔧 Scope: ${q.scope.join(", ")}`);
              if ((q.services ?? []).length > 0) {
                const svcLines = q.services.map(
                  (s) =>
              ...
              if ((q.modules ?? []).length > 0)
                descParts.push(
                  `🧩 Modules: ${q.modules.map((m) => m.moduleName).join(", ")}`,
                );
              if ((q.deliverables ?? []).length > 0)
                descParts.push(`📦 Deliverables: ${q.deliverables.join(", ")}`);
              if (q.paymentTerms)
                descParts.push(`💳 Payment Terms: ${q.paymentTerms}`);
            ...
            console.error(
              "Failed to auto-create journey event on update:",
              journeyErr,
            );
          }
        }
