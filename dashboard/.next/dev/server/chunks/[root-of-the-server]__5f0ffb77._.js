...
async function PUT(request, { params }) {
    try {
        ...
        // Fetch the existing quotation to detect status changes
        ...
        const updated = await __TURBOPACK__imported__module__$5b$project$5d2f$Desktop$2f$Projects$2f$final$2d$pixelate$2f$dashboard$2f$src$2f$lib$2f$services$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["updateById"]('quotations', id, {
            ...body,
            updatedAt: new Date()
        });
        // ── Auto-create Journey event when status transitions out of DRAFT ──
        const prevStatus = existing?.status ?? 'DRAFT';
        const newStatus = body.status ?? prevStatus;
        const clientId = existing?.clientId ?? body.clientId;
        if (clientId && prevStatus === 'DRAFT' && newStatus !== 'DRAFT') {
            try {
                const db = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$Desktop$2f$Projects$2f$final$2d$pixelate$2f$dashboard$2f$src$2f$lib$2f$mongodb$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["getDb"])();
                // Avoid duplicate: check if a journey event already exists for this quotation
                const alreadyExists = await db.collection('journey_events').findOne({
                    'metadata.quotationId': id
                });
                if (!alreadyExists) {
                    // Merge existing doc with incoming body for full data
                    const q = {
                        ...existing,
                        ...body
                    };
                    ...
                    if (!clientName && clientId) {
                        ...
                        try {
                            clientDoc = await db.collection('clients').findOne({
                                _id: new __TURBOPACK__imported__module__$5b$externals$5d2f$mongodb__$5b$external$5d$__$28$mongodb$2c$__cjs$29$__["ObjectId"](String(clientId))
                            });
                        } catch  {}
                    ...
                    const journeyStatus = newStatus === 'SENT' ? 'Sent' : newStatus === 'APPROVED' ? 'Approved' : newStatus === 'REJECTED' ? 'Rejected' : newStatus === 'CONVERTED' ? 'Completed' : 'Pending';
                    const quoteId = q.quoteId ?? id;
                    const title = q.title ?? '';
                    // Compute grand total
                    const grandTotal = (q.services ?? []).reduce((sum, s)=>sum + (s.price ?? 0) * (s.qty ?? 1), 0);
                    // Build rich description
                    const descParts = [];
                    descParts.push(`📄 Quote ID: ${quoteId}`);
                    if (q.subtitle) descParts.push(`📝 ${q.subtitle}`);
                    if (q.objective) descParts.push(`🎯 Objective: ${q.objective}`);
                    if ((q.scope ?? []).length > 0) descParts.push(`🔧 Scope: ${q.scope.join(', ')}`);
                    if ((q.services ?? []).length > 0) {
                        const svcLines = q.services.map((s)=>`${s.serviceName} × ${s.qty} @ ₹${(s.price ?? 0).toLocaleString('en-IN')}`);
                    ...
                    if ((q.timeline ?? []).length > 0) {
                        const tLines = q.timeline.map((t)=>`${t.phase} – ${t.duration}`);
                        descParts.push(`🗓 Timeline:\n${tLines.map((l)=>`  • ${l}`).join('\n')}`);
                    }
                    if ((q.modules ?? []).length > 0) descParts.push(`🧩 Modules: ${q.modules.map((m)=>m.moduleName).join(', ')}`);
                    if ((q.deliverables ?? []).length > 0) descParts.push(`📦 Deliverables: ${q.deliverables.join(', ')}`);
                    if (q.paymentTerms) descParts.push(`💳 Payment Terms: ${q.paymentTerms}`);
                    if (q.notes) descParts.push(`📌 Notes: ${q.notes}`);
                    await db.collection('journey_events').insertOne({
                        ...
                        type: 'quotation',
                        title: `Quotation Sent – ${title || quoteId}`,
                        description: descParts.join('\n\n'),
                        performedBy: 'System',
                        status: journeyStatus,
                        fileUrl: null,
                        linkUrl: `/quotations/${id}/view`,
                        occurredAt: new Date(),
                        metadata: {
                            quotationId: id,
                            quoteId,
                            ...
                            modulesCount: (q.modules ?? []).length,
                            scopeCount: (q.scope ?? []).length,
                            timelinePhases: (q.timeline ?? []).length,
                            status: newStatus
                        },
                        createdAt: new Date()
                    });
...
//# sourceMappingURL=%5Broot-of-the-server%5D__5f0ffb77._.js.map