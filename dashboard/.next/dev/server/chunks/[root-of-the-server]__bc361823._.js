...
"[project]/src/lib/bihar-cities.json (json)", ((__turbopack_context__) => {

__turbopack_context__.v({"_comment":"Allowlist of Bihar cities/districts. Matching is case-insensitive and whitespace-tolerant (see normalize() in consumers). Add new spellings/variants as you encounter them. Used by dashboard/scripts/delete-non-bihar-kalahanu-leads.js and dashboard/src/app/api/client-leads/sync/route.ts.","cities":["Bihar","Araria","Arwal","Aurangabad","Banka","Begusarai","Bhagalpur","Bhojpur","Buxar","Darbhanga","East Champaran","Gaya","Gopalganj","Jamui","Jehanabad","Kaimur","Katihar","Khagaria","Kishanganj","Lakhisarai","Madhepura","Madhubani","Munger","Muzaffarpur","Nalanda","Nawada","Patna","Purnia","Purnea","Rohtas","Saharsa","Samastipur","Saran","Sheikhpura","Sheohar","Sitamarhi","Siwan","Supaul","Vaishali","West Champaran","Arrah","Ara","Motihari","Bettiah","Hajipur","Chhapra","Chapra","Sasaram","Dehri","Bihar Sharif","Biharsharif","Bihar sharif","Bhabua","Bhabhua","Bodh Gaya","Bodhgaya","Rajgir","Bagaha","Forbesganj","Jamalpur","Mokama","Mokameh","Barh","Danapur","Dinapur","Phulwari Sharif","Phulwarisharif","Khagaul","Maner","Bihta","Fatuha","Masaurhi","Naugachhia","Nawgachiya","Raxaul","Sonepur","Sonpur","Hilsa","Islampur","Jagdishpur","Piro","Pupri","Rosera","Sherghati","Tarapur","Teghra","Dumraon","Lalganj","Mahnar","Janakpur Road","Jhanjharpur","Benipur","Mehsi","Sugauli","Chakia","Areraj","Bairgania","Kesaria","Maharajganj","Birpur","Nirmali","Thakurganj","Pakaur","Barauni","Khusrupur","Punpun","Paliganj","Bikram","Nokha","Mohania","Daudnagar","Obra","Tikari","Sherghati","Imamganj","Wazirganj","Warisaliganj","Hisua","Rajauli","Sikandra","Pirpainti","Kahalgaon","Naugachia","Manihari","Barsoi","Pranpur","Korha","Amarpur","Belhar","Dhuraiya","Katoria","Sultanganj","Nathnagar","Goh","Madanpur","Rafiganj","Nabinagar","Kutumba"]});}),
"[project]/src/app/api/client-leads/sync/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
var __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$bihar$2d$cities$2e$json__$28$json$29$__ = __turbopack_context__.i("[project]/src/lib/bihar-cities.json (json)");
;
;
;
;
...
const BIHAR_SET = new Set(__TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$bihar$2d$cities$2e$json__$28$json$29$__["default"].cities.map(normalizeCity));
const isBiharCity = (v)=>v != null && BIHAR_SET.has(normalizeCity(v));
function extractCityFromFields(f) {
...
async function getTokenPermissions(token) {
    try {
        ...
        const data = await res.json();
        if (data.error) {
            console.warn("[LeadsSync] permissions error:", JSON.stringify(data.error));
            return {
                granted: [],
                declined: [],
                error: data.error
...
async function fetchAdAccountLeads(adAccountId, token) {
    ...
    while(nextUrl){
        try {
            ...
            const data = await res.json();
            if (data.error) {
                console.warn(`[LeadsSync] Ad account leads error [${adAccountId}] http=${res.status}:`, JSON.stringify(data.error));
            ...
            console.warn(`[LeadsSync] Ad account leads fetch threw [${adAccountId}]:`, e?.message);
...
async function fetchPageFormLeads(pageId, pageToken) {
    const leads = [];
    try {
        const formsRes = await fetch(`${GRAPH}/${pageId}/leadgen_forms?fields=id,name,status&limit=50&access_token=${pageToken}`);
        const formsData = await formsRes.json();
        if (formsData.error) return leads;
        for (const form of formsData.data || []){
            let nextUrl = `${GRAPH}/${form.id}/leads?fields=id,created_time,ad_id,ad_name,adset_id,adset_name,campaign_id,campaign_name,field_data&limit=100&access_token=${pageToken}`;
            while(nextUrl){
                try {
                    const res = await fetch(nextUrl);
                    const data = await res.json();
...
function extractPhone(f) {
    return f["phone_number"] || f["phone"] || f["mobile_number"] || f["contact_number"] || f["mobile"] || f["whatsapp_number"] || f["Phone Number"] || f["Phone"] || "";
}
    ...
    try {
        ...
        try {
            client = await clientsCol.findOne({
                _id: new __TURBOPACK__imported__module__$5b$externals$5d2f$mongodb__$5b$external$5d$__$28$mongodb$2c$__cjs$29$__["ObjectId"](clientId)
            });
        } catch  {
            return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
        ...
        const leadsCol = await __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$services$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["getCollection"]("leads");
        let synced = 0;
        let skipped = 0;
        let filteredNonBihar = 0;
        const allRawLeads = [];
        const seenMetaIds = new Set();
        const fbAdsCol = await __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$services$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["getCollection"]("fbAdsConnections");
        const fbConn = await fbAdsCol.findOne({
            clientId
        });
        const fbToken = fbConn?.accessToken || token;
        const fbTokenSource = fbConn?.accessToken ? "fbAdsConnection.accessToken" : "client.metaAccessToken";
        ...
        const hasAdsRead = permInfo.granted.includes("ads_read") || permInfo.granted.includes("ads_management");
        const hasLeadsRetrieval = permInfo.granted.includes("leads_retrieval");
        console.log(`[LeadsSync] Token permissions — granted: [${permInfo.granted.join(", ")}], declined: [${permInfo.declined.join(", ")}]`);
        console.log(`[LeadsSync] ads_read/ads_management present: ${hasAdsRead}; leads_retrieval present: ${hasLeadsRetrieval}`);
        const adAccounts = [];
        let adAccountDiscoveryError = undefined;
        if (fbConn?.adAccountId) {
            const storedId = fbConn.adAccountId.startsWith("act_") ? fbConn.adAccountId : `act_${fbConn.adAccountId}`;
            adAccounts.push({
                id: storedId,
                name: storedId
            });
            ...
            console.log(`[LeadsSync] Discovered ${accounts.length} ad accounts via token${error ? ` (error: ${JSON.stringify(error)})` : ""}`);
        }
        const adAccountStats = [];
        for (const adAccount of adAccounts){
            const { leads, error, httpStatus } = await fetchAdAccountLeads(adAccount.id, fbToken);
            console.log(`[LeadsSync] Ad account ${adAccount.name} (${adAccount.id}): ${leads.length} leads, http=${httpStatus ?? "n/a"}${error ? `, error=${JSON.stringify(error)}` : ""}`);
            adAccountStats.push({
                id: adAccount.id,
                name: adAccount.name,
        ...
        let pages = [];
        try {
            pages = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$meta$2d$api$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["getUserPages"])(fbToken);
        } catch  {}
        for (const page of pages){
        ...
        for (const ml of allRawLeads){
            ...
            if (phone || email) {
                const orClauses = [];
                if (phone) orClauses.push({
                    phone
                });
                if (email) orClauses.push({
                    email
                });
...
//# sourceMappingURL=%5Broot-of-the-server%5D__bc361823._.js.map