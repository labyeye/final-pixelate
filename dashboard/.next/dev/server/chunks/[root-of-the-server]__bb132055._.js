...
async function GET(request) {
    try {
        ...
        const messagesByStatus = {
            sent: messages.filter((m)=>m.deliveryStatus === "sent" || m.status === "sent" && !m.deliveryStatus).length,
            delivered: messages.filter((m)=>m.deliveryStatus === "delivered" || m.status === "delivered").length,
            read: messages.filter((m)=>m.deliveryStatus === "read" || m.status === "read").length,
            failed: messages.filter((m)=>m.deliveryStatus === "failed" || m.status === "failed").length
        };
        ...
        const campaignInsights = campaigns.reduce((acc, campaign)=>({
        ...
        const deliveredMessages = messages.filter((m)=>m.deliveryStatus === "delivered" || m.status === "delivered");
        ...
        const insights = {
            ...
            metrics: {
                ...
                pendingRate: sentCount > 0 ? (pendingCount / sentCount * 100).toFixed(1) : "0"
            }
        };
...
//# sourceMappingURL=%5Broot-of-the-server%5D__bb132055._.js.map