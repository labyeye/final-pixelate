      ...
      try {
        ...
        if (newQuotation.clientId && newQuotation.status !== "DRAFT") {
          try {
            ...
            const grandTotal = (newQuotation.services ?? []).reduce(
              (sum, s) => sum + (s.price ?? 0) * (s.qty ?? 1),
              0,
            );
            const journeyStatus =
              newQuotation.status === "SENT"
                ? "Sent"
                : newQuotation.status === "APPROVED"
                  ? "Approved"
                  : newQuotation.status === "REJECTED"
                    ? "Rejected"
                    : newQuotation.status === "CONVERTED"
                      ? "Completed"
                      : "Pending";

            const descParts = [];
            descParts.push(`📄 Quote ID: ${newQuotation.quoteId}`);
            if (newQuotation.subtitle)
              descParts.push(`📝 ${newQuotation.subtitle}`);
            if (newQuotation.objective)
              descParts.push(`🎯 Objective: ${newQuotation.objective}`);
            if ((newQuotation.scope ?? []).length > 0)
              descParts.push(`🔧 Scope: ${newQuotation.scope.join(", ")}`);
            if ((newQuotation.services ?? []).length > 0) {
              const svcLines = newQuotation.services.map(
                (s) =>
                  `${s.serviceName} × ${s.qty} @ ₹${(s.price ?? 0).toLocaleString("en-IN")}`,
              );
              descParts.push(
            ...
            if ((newQuotation.timeline ?? []).length > 0) {
              const tLines = newQuotation.timeline.map(
            ...
            if ((newQuotation.modules ?? []).length > 0)
              descParts.push(
                `🧩 Modules: ${newQuotation.modules.map((m) => m.moduleName).join(", ")}`,
              );
            if ((newQuotation.deliverables ?? []).length > 0)
              descParts.push(
                `📦 Deliverables: ${newQuotation.deliverables.join(", ")}`,
              );
            if (newQuotation.paymentTerms)
              descParts.push(`💳 Payment Terms: ${newQuotation.paymentTerms}`);
            if (newQuotation.notes)
              descParts.push(`📌 Notes: ${newQuotation.notes}`);
