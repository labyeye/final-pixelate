...
import { Card, Row, StatusBadge, EmptyState, LoadingSpinner } from '../../components/common';
import { Colors, Typography, Spacing, Border } from '../../theme';
import { format } from 'date-fns';

const NDAApprovalScreen = () => {
  const { data: ndas = [], isLoading } = useQuery({ queryKey: ['nda'], queryFn: () => ndaAPI.getAll().then(r => r.data) });
    ...
    <SafeAreaView style={styles.safe} edges={['bottom']}>
      <View style={styles.container}>
        <View style={styles.infoBanner}>
          ...
          <Text style={styles.infoBannerSub}>Non-Disclosure Agreements awaiting review</Text>
        </View>
        <Text style={styles.count}>{items.length} RECORDS</Text>
        <FlatList
          data={items}
          keyExtractor={(item, i) => String(item._id || i)}
          ListEmptyComponent={<EmptyState title="No NDA Requests" subtitle="NDA approval requests will appear here" />}
          renderItem={({ item }) => (
            <Card style={[styles.card, { borderLeftColor: Colors.gray700, borderLeftWidth: 4 }]} shadow="sm">
...
const styles = StyleSheet.create({
  ...
  infoBanner: { backgroundColor: Colors.gray900, padding: Spacing.md, marginBottom: Spacing.base, borderWidth: Border.widthBold, borderColor: Colors.border },
  infoBannerText: { fontSize: Typography.sm, fontWeight: Typography.black, color: Colors.white, letterSpacing: 1 },
  infoBannerSub: { fontSize: Typography.sm, color: Colors.gray400, fontWeight: Typography.medium, marginTop: 2 },
  ...
  project: { fontSize: Typography.sm, color: Colors.mutedForeground, fontWeight: Typography.medium, marginTop: 1 },
