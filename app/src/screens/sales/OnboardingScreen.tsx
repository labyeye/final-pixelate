...
const OnboardingScreen = () => {
  ...
  const { data: onboarding = [], isLoading } = useQuery({ queryKey: ['onboarding'], queryFn: () => onboardingAPI.getAll().then(r => r.data) });

  if (isLoading) return <LoadingSpinner />;

  const filtered = Array.isArray(onboarding)
    ? onboarding.filter((o: any) => !search || o.clientName?.toLowerCase().includes(search.toLowerCase()))
    : [];
    ...
    <SafeAreaView style={styles.safe} edges={['bottom']}>
      <View style={styles.container}>
        ...
        <FlatList
          ...
          ListEmptyComponent={<EmptyState title="No Onboarding Records" subtitle="Onboarding documents will appear here" />}
          renderItem={({ item }) => (
...
const styles = StyleSheet.create({
  ...
  project: { fontSize: Typography.sm, color: Colors.primary, fontWeight: Typography.bold, marginTop: 2 },
