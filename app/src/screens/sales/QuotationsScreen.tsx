...
const QuotationsScreen = () => {
  ...
  const { data: quotations = [], isLoading } = useQuery({ queryKey: ['quotations'], queryFn: () => quotationsAPI.getAll().then(r => r.data) });

  if (isLoading) return <LoadingSpinner />;

  const filtered = Array.isArray(quotations)
    ? quotations.filter((q: any) => !search || q.clientName?.toLowerCase().includes(search.toLowerCase()))
    : [];
    ...
    <SafeAreaView style={styles.safe} edges={['bottom']}>
      <View style={styles.container}>
        ...
        <FlatList
          ...
          ListEmptyComponent={<EmptyState title="No Quotations" subtitle="Quotations will appear here" />}
          renderItem={({ item }) => (
            <TouchableOpacity onPress={() => navigation.navigate('QuotationDetail', { id: String(item._id || item.id) })} activeOpacity={0.85}>
              <Card style={[styles.card, { borderLeftColor: Colors.accent, borderLeftWidth: 4 }]} shadow="sm">
                <Row justify="space-between" align="flex-start">
                  <View style={styles.left}>
                    {item.quotationNumber ? <Text style={styles.qno}>QT-{item.quotationNumber}</Text> : null}
                    <Text style={styles.client}>{item.clientName}</Text>
                    {item.createdAt ? <Text style={styles.date}>{format(new Date(item.createdAt), 'dd MMM yyyy')}</Text> : null}
                  </View>
                  <View style={styles.right}>
                    <Text style={styles.total}>₹{(item.total || item.amount || 0).toLocaleString('en-IN')}</Text>
...
const styles = StyleSheet.create({
  ...
  client: { fontSize: Typography.base, fontWeight: Typography.black, color: Colors.foreground },
  date: { fontSize: Typography.sm, color: Colors.mutedForeground, fontWeight: Typography.medium, marginTop: 2 },
  total: { fontSize: Typography.xl, fontWeight: Typography.black, color: Colors.accent },
});
