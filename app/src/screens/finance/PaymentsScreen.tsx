...
const PaymentsScreen = () => {
  ...
  const [form, setForm] = useState({ clientName: '', amount: '', mode: 'Bank Transfer', date: '', notes: '' });
  ...
  const addMutation = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['payments'] }); setShowAdd(false); setForm({ clientName: '', amount: '', mode: 'Bank Transfer', date: '', notes: '' }); },
    onError: () => Alert.alert('Error', 'Failed to add payment'),
  });

  if (isLoading) return <LoadingSpinner />;

  const filtered = Array.isArray(payments) ? payments.filter((p: any) => !search || p.clientName?.toLowerCase().includes(search.toLowerCase())) : [];
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['bottom']}>
      <View style={styles.container}>
        ...
        <SearchBar value={search} onChangeText={setSearch} placeholder="Search payments..." />
        ...
        <FlatList
          ...
          ListEmptyComponent={<EmptyState title="No Payments" subtitle="Record incoming payments" action={{ label: '+ Add Payment', onPress: () => setShowAdd(true) }} />}
          renderItem={({ item }) => (
            <Card style={[styles.card, { borderLeftColor: Colors.success, borderLeftWidth: 4 }]} shadow="sm">
              <Row justify="space-between" align="flex-start">
                <View style={styles.left}>
                  <Text style={styles.clientName}>{item.clientName}</Text>
                  {item.mode ? <Text style={styles.mode}>via {item.mode}</Text> : null}
      ...
      <Modal visible={showAdd} animationType="slide" presentationStyle="pageSheet">
        <SafeAreaView style={styles.modal}>
          ...
          <View style={styles.modalContent}>
            <Input label="CLIENT NAME *" value={form.clientName} onChangeText={v => setForm(p => ({ ...p, clientName: v }))} placeholder="Client name" />
            ...
            <Input label="NOTES" value={form.notes} onChangeText={v => setForm(p => ({ ...p, notes: v }))} placeholder="Payment reference..." />
            <Text style={styles.pickLabel}>PAYMENT MODE</Text>
            <View style={styles.modeRow}>
              {['Bank Transfer', 'UPI', 'Cash', 'Cheque', 'Card'].map(m => (
                <TouchableOpacity key={m} style={[styles.modeChip, form.mode === m && styles.modeChipActive]} onPress={() => setForm(p => ({ ...p, mode: m }))}>
                  <Text style={[styles.modeChipText, form.mode === m && styles.modeChipTextActive]}>{m}</Text>
                </TouchableOpacity>
              ))}
            </View>
            <Button label={addMutation.isPending ? 'ADDING...' : 'ADD PAYMENT'} onPress={() => { if (!form.clientName.trim() || !form.amount) { Alert.alert('Error', 'Client and amount required'); return; } addMutation.mutate({ ...form, amount: Number(form.amount) }); }} loading={addMutation.isPending} fullWidth size="lg" style={{ marginTop: Spacing.base }} />
          </View>
        </SafeAreaView>
      </Modal>
    </SafeAreaView>
...
const styles = StyleSheet.create({
  ...
  summaryCard: { marginBottom: Spacing.sm, padding: Spacing.md, backgroundColor: Colors.success, borderColor: Colors.black },
  ...
  mode: { fontSize: Typography.sm, color: Colors.accent, fontWeight: Typography.bold, marginTop: 2 },
  ...
  amount: { fontSize: Typography.xl, fontWeight: Typography.black, color: Colors.success },
