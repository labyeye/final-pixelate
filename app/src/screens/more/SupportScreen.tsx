...
const SupportScreen = () => {
  ...
  const [form, setForm] = useState({ subject: '', description: '', priority: 'medium', clientName: '' });

  const { data: tickets = [], isLoading } = useQuery({ queryKey: ['support-tickets'], queryFn: () => supportAPI.getAll().then(r => r.data) });

  const addMutation = useMutation({
    mutationFn: (data: any) => supportAPI.create(data),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['support-tickets'] }); setShowAdd(false); setForm({ subject: '', description: '', priority: 'medium', clientName: '' }); },
  ...
  const filtered = Array.isArray(tickets)
    ? tickets.filter((t: any) => {
        const ms = !search || t.subject?.toLowerCase().includes(search.toLowerCase()) || t.clientName?.toLowerCase().includes(search.toLowerCase());
    ...
    <SafeAreaView style={styles.safe} edges={['bottom']}>
      <View style={styles.container}>
        ...
        <FlatList
          ...
          ListEmptyComponent={<EmptyState title="No Tickets" subtitle="Support tickets will appear here" action={{ label: '+ Create Ticket', onPress: () => setShowAdd(true) }} />}
          renderItem={({ item }) => (
            <TouchableOpacity onPress={() => navigation.navigate('SupportDetail', { id: String(item._id || item.id) })} activeOpacity={0.85}>
              <Card style={[styles.card, { borderLeftColor: priorityColor(item.priority), borderLeftWidth: 4 }]} shadow="sm">
      ...
      <Modal visible={showAdd} animationType="slide" presentationStyle="pageSheet">
        <SafeAreaView style={styles.modal}>
          ...
          <View style={styles.modalContent}>
            <Input label="SUBJECT *" value={form.subject} onChangeText={v => setForm(p => ({ ...p, subject: v }))} placeholder="Describe the issue briefly" />
            <Input label="CLIENT NAME" value={form.clientName} onChangeText={v => setForm(p => ({ ...p, clientName: v }))} placeholder="Client company" />
            <Input label="DESCRIPTION" value={form.description} onChangeText={v => setForm(p => ({ ...p, description: v }))} placeholder="Detailed description..." multiline />
            ...
            <Button label={addMutation.isPending ? 'CREATING...' : 'CREATE TICKET'} onPress={() => { if (!form.subject.trim()) { Alert.alert('Error', 'Subject required'); return; } addMutation.mutate({ ...form, status: 'open' }); }} loading={addMutation.isPending} fullWidth size="lg" style={{ marginTop: Spacing.base }} />
          </View>
        </SafeAreaView>
      </Modal>
    </SafeAreaView>
...
const styles = StyleSheet.create({
  ...
  subject: { fontSize: Typography.base, fontWeight: Typography.black, color: Colors.foreground },
