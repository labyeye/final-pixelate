...
import { Card, Row, Button, Input, Divider, LoadingSpinner, EmptyState } from '../../components/common';
...
const AboutTeamScreen = () => {
  ...
  const [form, setForm] = useState({ name: '', designation: '', phone: '', order: '0' });
  ...
  const openEdit = (item: any) => {
    setEditing(item);
    setForm({ name: item.name || '', designation: item.designation || '', phone: item.phone || '', order: String(item.order || 0) });
    setShowModal(true);
  };
  ...
  const list = Array.isArray(members) ? [...members].sort((a: any, b: any) => (a.order || 0) - (b.order || 0)) : [];
    ...
    <SafeAreaView style={styles.safe} edges={['bottom']}>
      <View style={styles.container}>
        ...
        <FlatList
          ...
          ListEmptyComponent={<EmptyState title="No Team Members" subtitle="Add team members for the About Us page" />}
          renderItem={({ item }) => (
              ...
              <Row justify="space-between" align="flex-start">
                <View style={styles.info}>
                  ...
                  <View style={styles.orderBadge}>
                    <Text style={styles.orderText}>ORDER #{item.order || 0}</Text>
                  </View>
                </View>
      ...
      <Modal visible={showModal} animationType="slide" presentationStyle="pageSheet">
        <SafeAreaView style={styles.modal}>
          ...
          <ScrollView style={styles.modalContent}>
            <Input label="NAME *" value={form.name} onChangeText={t => setForm(f => ({ ...f, name: t }))} placeholder="Full name" />
            <Input label="DESIGNATION" value={form.designation} onChangeText={t => setForm(f => ({ ...f, designation: t }))} placeholder="e.g. Creative Director" />
            <Input label="PHONE" value={form.phone} onChangeText={t => setForm(f => ({ ...f, phone: t }))} placeholder="+91 9999999999" keyboardType="phone-pad" />
            <Input label="ORDER" value={form.order} onChangeText={t => setForm(f => ({ ...f, order: t }))} placeholder="Display order" keyboardType="numeric" />
            <Button
...
const styles = StyleSheet.create({
  ...
  avatar: { width: 44, height: 44, backgroundColor: Colors.primary, borderWidth: Border.width, borderColor: Colors.black, alignItems: 'center', justifyContent: 'center' },
  ...
  designation: { fontSize: Typography.sm, color: Colors.primary, fontWeight: Typography.bold, marginTop: 1 },
  phone: { fontSize: Typography.xs, color: Colors.mutedForeground, fontWeight: Typography.medium, marginTop: 1 },
  orderBadge: { marginTop: Spacing.xs, alignSelf: 'flex-start', backgroundColor: Colors.gray200, paddingHorizontal: 8, paddingVertical: 2 },
  orderText: { fontSize: Typography.xs, fontWeight: Typography.black, color: Colors.mutedForeground, letterSpacing: 0.5 },
  actions: { gap: 6 },
  editBtn: { paddingHorizontal: 10, paddingVertical: 6, borderWidth: Border.width, borderColor: Colors.primary },
  editBtnText: { fontSize: Typography.xs, fontWeight: Typography.black, color: Colors.primary },
