...
import { Card, Row, Button, Input, EmptyState, LoadingSpinner, SectionHeader } from '../../components/common';
...
const JourneyScreen = () => {
  ...
  const [form, setForm] = useState({ title: '', description: '', date: '', clientName: '' });

  const { data: journey = [], isLoading } = useQuery({ queryKey: ['journey'], queryFn: () => journeyAPI.getAll().then(r => r.data) });

  const addMutation = useMutation({
    mutationFn: (data: any) => journeyAPI.create(data),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['journey'] }); setShowAdd(false); setForm({ title: '', description: '', date: '', clientName: '' }); },
    ...
    <SafeAreaView style={styles.safe} edges={['bottom']}>
      <View style={styles.container}>
        ...
        <FlatList
          ...
          ListEmptyComponent={<EmptyState title="No Journey Entries" subtitle="Document your client journey milestones" action={{ label: '+ Add Entry', onPress: () => setShowAdd(true) }} />}
      ...
      <Modal visible={showAdd} animationType="slide" presentationStyle="pageSheet">
        <SafeAreaView style={styles.modal}>
          ...
          <View style={styles.modalContent}>
            <Input label="TITLE *" value={form.title} onChangeText={v => setForm(p => ({ ...p, title: v }))} placeholder="e.g. Project kickoff" />
            ...
            <Input label="DATE (YYYY-MM-DD)" value={form.date} onChangeText={v => setForm(p => ({ ...p, date: v }))} placeholder="2025-01-15" />
            <Button label={addMutation.isPending ? 'ADDING...' : 'ADD MILESTONE'} onPress={() => { if (!form.title.trim()) { Alert.alert('Error', 'Title required'); return; } addMutation.mutate({ ...form, date: form.date || new Date().toISOString() }); }} loading={addMutation.isPending} fullWidth size="lg" style={{ marginTop: Spacing.base }} />
          </View>
        </SafeAreaView>
      </Modal>
    </SafeAreaView>
...
const styles = StyleSheet.create({
  ...
  dot: { width: 14, height: 14, backgroundColor: Colors.primary, borderWidth: Border.width, borderColor: Colors.border, borderRadius: 7, marginTop: 18 },
  ...
  client: { fontSize: Typography.sm, color: Colors.primary, fontWeight: Typography.bold, marginTop: 2 },
  desc: { fontSize: Typography.sm, color: Colors.mutedForeground, fontWeight: Typography.medium, marginTop: 4, lineHeight: 18 },
  date: { fontSize: Typography.xs, color: Colors.accent, fontWeight: Typography.bold, marginTop: 6, letterSpacing: 0.3 },
