...
const ServicesScreen = () => {
  ...
  const addMutation = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['services'] }); setShowAdd(false); setForm({ name: '', description: '', price: '', category: '' }); },
  ...
  const filtered = Array.isArray(services)
    ? services.filter((s: any) => !search || s.name?.toLowerCase().includes(search.toLowerCase()))
    : [];
    ...
    <SafeAreaView style={styles.safe} edges={['bottom']}>
      <View style={styles.container}>
        ...
        <FlatList
          ...
          ListEmptyComponent={<EmptyState title="No Services" subtitle="Add services to your catalogue" />}
          renderItem={({ item }) => (
      ...
      <Modal visible={showAdd} animationType="slide" presentationStyle="pageSheet">
        <SafeAreaView style={styles.modal}>
          ...
          <View style={styles.modalContent}>
            <Input label="SERVICE NAME *" value={form.name} onChangeText={v => setForm(p => ({ ...p, name: v }))} placeholder="e.g. Website Development" />
            <Input label="CATEGORY" value={form.category} onChangeText={v => setForm(p => ({ ...p, category: v }))} placeholder="e.g. Web Design" />
            <Input label="DESCRIPTION" value={form.description} onChangeText={v => setForm(p => ({ ...p, description: v }))} placeholder="Service details..." multiline />
            <Input label="PRICE (₹)" value={form.price} onChangeText={v => setForm(p => ({ ...p, price: v }))} keyboardType="numeric" />
            <Button label={addMutation.isPending ? 'ADDING...' : 'ADD SERVICE'} onPress={() => { if (!form.name.trim()) { Alert.alert('Error', 'Name required'); return; } addMutation.mutate({ ...form, price: Number(form.price) }); }} loading={addMutation.isPending} fullWidth size="lg" style={{ marginTop: Spacing.base }} />
          </View>
        </SafeAreaView>
      </Modal>
    </SafeAreaView>
...
const styles = StyleSheet.create({
  ...
  cardLeft: { flex: 1 },
  serviceName: { fontSize: Typography.base, fontWeight: Typography.black, color: Colors.foreground },
  ...
  priceBadge: { backgroundColor: Colors.success, paddingHorizontal: 10, paddingVertical: 4, borderWidth: Border.width, borderColor: Colors.border },
  priceText: { fontSize: Typography.sm, fontWeight: Typography.black, color: Colors.white },
