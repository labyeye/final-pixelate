...
const WhatsAppInboxScreen = () => {
  ...
  const { data: messages = [], isLoading } = useQuery({ queryKey: ['whatsapp-inbox'], queryFn: () => whatsappAPI.getInbox().then(r => r.data) });

  if (isLoading) return <LoadingSpinner />;

  const items = Array.isArray(messages) ? messages : [];
  const filtered = items.filter((m: any) => !search || m.from?.toLowerCase().includes(search.toLowerCase()) || m.name?.toLowerCase().includes(search.toLowerCase()));

  
  const conversations = filtered.reduce((acc: Record<string, any[]>, msg: any) => {
    ...
    <SafeAreaView style={styles.safe} edges={['bottom']}>
      <View style={styles.container}>
        ...
        <FlatList
          ...
          ListEmptyComponent={<EmptyState title="No Messages" subtitle="WhatsApp messages will appear here" />}
          renderItem={({ item }) => (
                ...
                <View style={styles.convInfo}>
                  <Row justify="space-between" align="center">
                    ...
                    {item.lastMsg?.timestamp ? (
                      <Text style={styles.convTime}>{format(new Date(item.lastMsg.timestamp * 1000 || item.lastMsg.timestamp), 'HH:mm')}</Text>
                    ) : null}
                  </Row>
                  <Text style={styles.lastMsg} numberOfLines={1}>
                    {item.lastMsg?.text?.body || item.lastMsg?.body || item.lastMsg?.message || '...'}
                  </Text>
                </View>
                {item.count > 1 ? (
...
const styles = StyleSheet.create({
  ...
  avatar: { width: 44, height: 44, backgroundColor: '#25D366', borderWidth: Border.width, borderColor: Colors.border, borderRadius: 22, alignItems: 'center', justifyContent: 'center' },
  avatarText: { fontSize: Typography.xl, fontWeight: Typography.black, color: Colors.white },
  convInfo: { flex: 1 },
  convName: { fontSize: Typography.base, fontWeight: Typography.black, color: Colors.foreground },
  convTime: { fontSize: Typography.xs, color: Colors.mutedForeground, fontWeight: Typography.medium },
  lastMsg: { fontSize: Typography.sm, color: Colors.mutedForeground, fontWeight: Typography.medium, marginTop: 2 },
  countBadge: { backgroundColor: '#25D366', width: 24, height: 24, borderRadius: 12, alignItems: 'center', justifyContent: 'center' },
  countText: { fontSize: Typography.xs, fontWeight: Typography.black, color: Colors.white },
});
