...
const BulkMessagingScreen = () => {
  ...
  const handleSend = async () => {
    ...
    if (selectedClients.length === 0) {
      Alert.alert('Error', 'Select at least one client');
      return;
    }
    setSending(true);
    try {
      ...
      Alert.alert(
        'Error',
        'Failed to send messages. Check your WhatsApp integration.',
      );
    } finally {
      setSending(false);
    }
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['bottom']}>
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <SectionHeader
          title={`SELECT RECIPIENTS (${selectedClients.length} selected)`}
        ...
        <Row style={{ marginBottom: Spacing.sm }} gap={8}>
          <TouchableOpacity
            ...
            onPress={() =>
              setSelectedClients(
                clientList.map((c: any) => String(c._id || c.id)),
              )
            }
          >
